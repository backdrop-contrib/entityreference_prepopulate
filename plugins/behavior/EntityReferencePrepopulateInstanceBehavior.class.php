...
class EntityReferencePrepopulateInstanceBehavior extends EntityReferenceBehaviorHandler {
...
function theme_entityreference_prepopulate_providers_table($variables) {
  ...
  foreach ($provider_names as $provider_name) {
    $row = array(
      'data' => array(
        backdrop_render($form['title'][$provider_name]),
        backdrop_render($form['enabled'][$provider_name]),
        backdrop_render($form['weight'][$provider_name]),
      ),
  ...
  backdrop_add_tabledrag('table-providers', 'order', 'sibling', 'provider-weight');
  return $output;
}
